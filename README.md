...
Сервис `app` подхватывает переменные из файла `.env` в корне репозитория (скопируйте из `.env.example`, задайте как минимум `OPENAI_API_KEY`). В `docker-compose.yml` для контейнера принудительно заданы `MILVUS_HOST=milvus`, `MILVUS_PORT=19530` и `KB_JSON_PATH=/app/mock_kb.json`, чтобы не затирать локальные значения из `.env` при работе в сети compose.
